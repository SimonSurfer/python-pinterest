...
class PinterestAPI(object):
    ...
    def login(self, username, password):
        """Obtain an OAuth access token to be used for this session."""
        path = "/v1/login/?client_id=1431598&timestamp=%d&oauth_signature=%s"

        # XXX: Does not work currently; need to figure out how to generate
        # the oauth_signature parameter.
        raise NotImplementedError()

        timestamp = int(time.time())
        oauth_sig = ""
        path = path % (timestamp, oauth_sig)

        data = {"username": username, "password": password}
        response = self._post_request(path, data=data)
        content = json.loads(response.text)

        if response.status_code != 200:
            raise PinterestException(content["error"])
    ...
    def all(self, category=None, page=1, limit=25):
        ...
        """
        path = "/v2/all"
        return self._get_request(path, {"category": category, "page": page, "limit": limit})
    ...
    def search(self, query, domain="pins", page=1, limit=25):
        ...
        return self._get_request(path, {"query": query, "page": page, "limit": limit})
    ...
    def repin(self, pin_id, board_id, details=None, publish_facebook=False, publish_twitter=False):
    ...
    def create_board(self, name, category=None, description=None):
        ...
        return self._post_request(path, data={"name": name, "category": category, "description": description})
